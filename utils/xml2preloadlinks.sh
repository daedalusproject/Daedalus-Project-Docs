...
if [ -f $SITEMAP_XML ]; then
	...
	for location in $locations; do
		...
		if [ -f $INDEXTOPROCESS ]; then
			rm -f $LINKS
			for link in $(grep -o "src=[^>]*.js>" $INDEXTOPROCESS | sed 's/^src=\([^>]*\)>$/\1/'); do
				echo "        add_header Link \"<$link>; rel=preload; as=script\";"
			done >$LINKS
			for link in $(grep -o "href=[^>]*.css" $INDEXTOPROCESS | sed 's/^href=\([^>]*\)$/\1/'); do
				echo "        add_header Link \"<$link>; rel=preload; as=style\";"
			done >>$LINKS
			for link in $(grep -o "href=[^>]*type=image" $INDEXTOPROCESS | sed 's/^href=\([^ ]*\) type=image$/\1/'); do
				echo "        add_header Link \"<$link>; rel=preload; as=image\";"
			done >>$LINKS
			for link in $(grep -o "img src=[^ ]*" $INDEXTOPROCESS | sed 's/^img src=\([^ ]*\)$/\1/'); do
				echo "        add_header Link \"<$link>; rel=preload; as=image\";"
			done >>$LINKS

			# Fonts
			echo "        add_header Link \"</fonts/Novecentosanswide-UltraLight-webfont.woff2>; rel=preload; as=font\";" >>$LINKS
			echo "        add_header Link \"</fonts/Lato-Regular.woff2>; rel=preload; as=font\";" >>$LINKS
			echo "        add_header Link \"</fonts/fontawesome-webfont.woff2?v=4.7.0>; rel=preload; as=font\";" >>$LINKS
			echo "        add_header Link \"</fonts/Lato-Bold.woff2>; rel=preload; as=font\";" >>$LINKS
